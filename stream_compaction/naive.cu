...
namespace StreamCompaction {
    namespace Naive {
        ...
        // TODO: __global__
        __global__ void kernNaiveScan(int n, int lim, int *odata, int *idata) {
        ...
        __global__ void kernNaiveScanShift(int n, int* odata, int* idata) {
            int index = (blockIdx.x * blockDim.x) + threadIdx.x;
            if (index >= n) return;
            odata[index] = (index == 0) ? 0 : idata[index - 1];
		}
        ...
        void scan(int n, int *odata, const int *idata) {
            ...
            if (n <= blockSize) {
                kernNaiveScan << <fullBlocksPerGrid, blockSize >> > (n, ilog2ceil(n), d_odata, d_idata);
            } else {
